...
- Cancellation eventually produces a visible terminal card state without deleting or replacing the card.
- Terminal agent failures stop remaining workflow work and wake the parent session with the failure.
- Terminal cards retain their existing phase, agent, model, schedule, and output metadata.
- Cancelled in-flight agents no longer appear to be running.
...
- External orchestrators create or update cards through `POST /session/:sessionID/agent-card`. Creating a card omits `messageID` and `partID`; later updates pass both identifiers returned by the create response so the same UI-only tool part changes in place without triggering an LLM turn.
...
- Workflow and schedule renderers own their terminal error presentation rather than falling back to the generic tool error card.
- Cancelled cards do not display exception details, and failed cards display a concise single-line error rather than a stack trace.
