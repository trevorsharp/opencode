...
## Directory Discovery Reliability

### Problem

The directory picker can appear empty when the primary search engine fails or returns no directories.

### Expected Behavior

- Primary failure or an empty result activates bounded fallback discovery.
- Empty untyped searches behave like directory searches.
- Results contain directories only.
- Discovery checks direct children and one additional level.
- Hidden, protected, generated, dependency, and vendor directories are excluded.
- Non-empty queries use fuzzy ranking.
- Empty queries produce stable results.
