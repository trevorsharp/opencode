...
From the 2026-08-19 rebase onward the fork is carried as a single consolidated commit on top of the pinned upstream target rather than one commit per feature. Use this directory, not commit boundaries, to identify which behavior belongs to which feature.
...
`disable-native-task-tool.md` documents hiding the built-in delegation tool from model-visible catalogs while keeping the core runtime agnostic about replacements supplied by plugins.
