...
- The project ID is `workspace:<absolute-workspace-root>`, using the absolute root path with no other prefix or encoding.
- No `feature:` compatibility, aliasing, or reconciliation behavior is included.
- Records written under an earlier identity are migrated manually, once, outside this feature. The fork ships no automatic migration and no dual-read fallback.
...
- Project rename behavior and the retained Edit Project dialog are unchanged by this feature.
...
The worktree creation contract uses explicit modes for the two workspace paths: `workspace-root` creates an empty root and `workspace-member` creates or attaches a member. Omitting the mode retains ordinary Git worktree creation.

Creation uses the shared directory picker where a source directory is required. Its configured default directories remain searchable, and its create-workspace action stays at the bottom when a search has no results and uses sentence-case text.
...
- Clicking a desktop notification preserves the workspace root when it opens a legacy session route.
...
- Removal results use `workspaceRootRemoved` and `workspaceRootDirectory`; public contracts and internal identifiers do not call workspace roots features.
...
- Member reset.
- Workspace reset.
- General project editing. Its legacy entry point is hidden by `remove-project-editing.md`; this feature does not change the editor.
- Workspace enable or disable toggles.

External changes remain visible through inventory refresh.

Project rename remains outside this feature and retains its upstream behavior. Copy Path entry points are absent independently of workspace behavior.
...
- Changing project rename or Edit Project dialog internals.
