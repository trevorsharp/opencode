...
export function StatusPopoverBody(props: { shown: Accessor<boolean>; mcpOnly?: boolean }) {
  const sync = useSync()
  const global = useGlobal()
  const server = useServer()
  const platform = usePlatform()
  ...
  return (
    <div class="flex items-center gap-1 w-[360px] rounded-xl shadow-[var(--shadow-lg-border-base)]">
      <Tabs
        ...
        class="tabs bg-background-strong rounded-xl overflow-hidden"
        data-component="tabs"
        data-active={props.mcpOnly || settings.general.newLayoutDesigns() ? "mcp" : "servers"}
        defaultValue={props.mcpOnly || settings.general.newLayoutDesigns() ? "mcp" : "servers"}
        variant="alt"
      >
        <Tabs.List data-slot="tablist" class="bg-transparent border-b-0 px-4 pt-2 pb-0 gap-4 h-10">
          {!props.mcpOnly && !settings.general.newLayoutDesigns() && (
            <Tabs.Trigger value="servers" data-slot="tab" class="text-12-regular">
